...
var projectiles = []
var my_peer = null
var last_update = -1 #what is this used for
onready var node_players #= $camera/players #what are these nodes used for?
onready var node_projectiles #= $camera/projectiles #what are these nodes used for?
onready var camera = get_tree().get_root()
		...
		for i in range(0, keys.size()):
			if keys[i] > target_timestamp:
				if not player_info[peer_id].destroyed:
					...
					player_info[peer_id].rotation = Networking.lerp_angle(player_info[peer_id].updates[keys[i-1]].rotation, player_info[peer_id].updates[keys[i]].rotation, percent)
...
remote func player_joined(id, info): #tweak code
	...
	#remove or tweak the code line below
	node_player.get_node("texture_player").texture = load("res://New game code and features/multiplayer/images/player_" + color + ".png")
	
	info.node = node_player #info node?
	info.updates = {}
...
remote func player_health(id, health): #what is a remote function
...
remote func fire_weapon(id, position, current_angle): #update code to be an attack #inhumanity
...
func add_chat(text):
	#fix this function
	chat.add_item(text) #breaks here. returns nill
